...
{
    /// <summary>
    /// Command to convert PNG and JPEG images to WebP format.
    /// Supports single files, folders, projects, and solutions.
    /// </summary>
    [Command(PackageGuids.guidImageOptimizerCmdSetString, PackageIds.cmdConvertToWebp)]
    internal class ConvertToWebpCommand : BaseCommand<ConvertToWebpCommand>
    {
        ...
        /// <summary>
        /// Gets all convertible image files from the selected Solution Explorer items.
        /// </summary>
            ...
            foreach (SolutionItem item in items)
            {
                switch (item.Type)
                {
                    case SolutionItemType.PhysicalFile:
                        if (Compressor.IsConvertibleToWebp(item.FullPath))
                        {
